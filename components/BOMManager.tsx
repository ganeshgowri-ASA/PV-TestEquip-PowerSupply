...
const FULL_BOM: BOMItem[] = [
  { id: 'b1', partNumber: 'PSU-TCHF-01', mpn: 'IT6512C', description: 'Itech IT6500C Bidirectional PSU 60V/30A', qty: 5, category: 'Power Supply', subsystem: 'TC/HF', primaryVendor: 'Itech Direct', indianVendor: 'Scientech, Mumbai', unitPriceInr: 285000, leadTimeWeeks: 6 },
  { id: 'b2', partNumber: 'PSU-LETID-01', mpn: 'E36312A', description: 'Keysight E36312A Triple Output 60V/2A', qty: 2, category: 'Power Supply', subsystem: 'LETID', primaryVendor: 'Keysight Direct', indianVendor: 'Scientech, Mumbai', unitPriceInr: 145000, leadTimeWeeks: 4 },
  ...
  { id: 'b9', partNumber: 'MB-01', mpn: 'USR-TCP232-410S', description: 'USR RS485 to Ethernet Converter', qty: 5, category: 'Communication', subsystem: 'Common', primaryVendor: 'USR IOT', indianVendor: 'Robu.in', unitPriceInr: 3500, leadTimeWeeks: 3 },
  { id: 'b10', partNumber: 'TS-01', mpn: 'PT100-A-3W', description: 'PT100 RTD Class A 3-Wire', qty: 20, category: 'Sensor', subsystem: 'Common', primaryVendor: 'Omega', indianVendor: 'Tempsens, Jaipur', unitPriceInr: 850, leadTimeWeeks: 2 },
  { id: 'b11', partNumber: 'TS-02', mpn: 'PT1000-B-4W', description: 'PT1000 RTD Class B 4-Wire', qty: 10, category: 'Sensor', subsystem: 'LETID', primaryVendor: 'Omega', indianVendor: 'Tempsens, Jaipur', unitPriceInr: 1200, leadTimeWeeks: 2 },
  { id: 'b12', partNumber: 'HS-01', mpn: 'SHT45', description: 'Sensirion SHT45 Humidity Sensor', qty: 5, category: 'Sensor', subsystem: 'Common', primaryVendor: 'DigiKey', indianVendor: 'Evelta Electronics', unitPriceInr: 950, leadTimeWeeks: 3 },
  { id: 'b13', partNumber: 'DAQ-01', mpn: 'DAQ970A', description: 'Keysight DAQ970A Data Logger', qty: 1, category: 'Data Acquisition', subsystem: 'Common', primaryVendor: 'Keysight', indianVendor: 'Scientech, Mumbai', unitPriceInr: 185000, leadTimeWeeks: 5 },
  { id: 'b14', partNumber: 'EX-01', mpn: '3842990720', description: 'Bosch Rexroth 45x45 Profile 2m', qty: 20, category: 'Mechanical', subsystem: 'Common', primaryVendor: 'Bosch Rexroth', indianVendor: 'Bosch Rexroth India', unitPriceInr: 1800, leadTimeWeeks: 3 },
  { id: 'b15', partNumber: 'HS-02', mpn: 'SK104-50.8', description: 'Fischer SK104 Heat Sink 50mm', qty: 20, category: 'Thermal', subsystem: 'TC/HF', primaryVendor: 'DigiKey', indianVendor: 'SP Robotics, Chennai', unitPriceInr: 320, leadTimeWeeks: 2 },
  ...
  { id: 'b27', partNumber: 'R-01', mpn: 'CSRN2512FK10L0', description: 'Current Sense 10mOhm 1% 2512', qty: 20, category: 'Passive', subsystem: 'Common', primaryVendor: 'DigiKey', indianVendor: 'Evelta Electronics', unitPriceInr: 25, leadTimeWeeks: 1 },
...
function leadTimeColor(weeks?: number): string {
  if (!weeks) return 'text-gray-500';
  if (weeks < 2) return 'text-green-400';
  if (weeks <= 8) return 'text-yellow-400';
  return 'text-red-400';
}

function leadTimeBg(weeks?: number): string {
  if (!weeks) return 'bg-gray-800';
...
export default function BOMManager() {
  ...
  const filtered = bomItems.filter(b => {
    const matchSubsystem = filter === 'All' || b.subsystem === filter;
    const matchSearch = searchQuery === '' ||
      b.description.toLowerCase().includes(searchQuery.toLowerCase()) ||
      b.mpn.toLowerCase().includes(searchQuery.toLowerCase()) ||
      b.partNumber.toLowerCase().includes(searchQuery.toLowerCase());
    return matchSubsystem && matchSearch;
  });

  const totalInr = filtered.reduce((sum, b) => sum + b.unitPriceInr * b.qty * rackCount, 0);
  const longestLeadTime = filtered.reduce((max, b) => Math.max(max, b.leadTimeWeeks || 0), 0);
  ...
  const handleNexarCheckAll = async () => {
    ...
    toast('success', 'Nexar price check complete (first 5 items)');
  };

  const handleQtyChange = (id: string, newQty: number) => {
    setBomItems(prev => prev.map(b => b.id === id ? { ...b, qty: Math.max(1, newQty) } : b));
  };
  ...
  const exportExcel = async () => {
    try {
      ...
      const data = filtered.map(item => ({
        ...
        'Lead Time (weeks)': item.leadTimeWeeks || 'N/A',
        'Nexar Price (INR)': item.mouserPrice || 'N/A',
      }));
      ...
      toast('error', 'Failed to export Excel (xlsx package may not be installed)');
    }
  };
  ...
  const exportPDF = async () => {
    try {
      ...
      autoTable(doc, {
        startY: 35,
        head: [['Part No.', 'MPN', 'Description', 'Qty', 'Subsystem', 'Vendor', 'Unit (INR)', 'Total (INR)', 'Lead (wk)']],
    ...
    <div className="space-y-6">
      <div className="flex items-start justify-between">
        <div>
          ...
          <p className="text-gray-400 text-sm">{bomItems.length} items | Indian vendor alternatives | Nexar API pricing</p>
        </div>
        <div className="text-right space-y-1">
          ...
          <p className="text-xs text-gray-500">Longest lead time: <span className={leadTimeColor(longestLeadTime)}>{longestLeadTime} weeks</span></p>
        </div>
      </div>
      ...
      <div className="flex gap-3 items-end flex-wrap">
        <div className="flex-1 min-w-[200px]">
          <Input
            placeholder="Search by part number, MPN, or description..."
            value={searchQuery}
      ...
      <div className="flex gap-2 items-center">
        <Button size="sm" variant="outline" className="text-xs h-7" onClick={() => setShowDistributors(!showDistributors)}>
          {showDistributors ? 'Hide' : 'Show'} Indian Distributors
        </Button>
        <Button size="sm" variant="outline" className="text-xs h-7" onClick={handleNexarCheckAll}>
          Nexar Check All (Top 5)
        </Button>
      </div>
      {showDistributors && (
        <div className="flex gap-2 flex-wrap">
          {INDIAN_DISTRIBUTORS.map(d => (
            <div key={d.name} className="px-3 py-1.5 bg-gray-800 rounded-lg border border-gray-700 text-xs">
              <span className="text-blue-300 font-medium">{d.name}</span>
              <span className="text-gray-500 ml-2">{d.url}</span>
            </div>
          ))}
        </div>
      )}
            ...
            <table className="w-full text-sm">
                ...
                {filtered.map((item) => (
                  <tr key={item.id} className="border-b border-gray-800 hover:bg-gray-800/50">
                    ...
                    <td className="p-3 text-center">
                      <Input
                        type="number"
                        value={item.qty * rackCount}
                        onChange={(e) => handleQtyChange(item.id, Math.round(Number(e.target.value) / rackCount))}
                    ...
                    <td className="p-3 text-center">
                      <span className={`text-xs font-mono px-2 py-0.5 rounded ${leadTimeBg(item.leadTimeWeeks)} ${leadTimeColor(item.leadTimeWeeks)}`}>
                    ...
                    <td className="p-3 text-center">
                      <div className="flex flex-col items-center gap-1">
                        ...
                        {item.mouserPrice && (
                          <span className="text-xs text-yellow-400 font-mono">{'\u20B9'}{item.mouserPrice.toLocaleString('en-IN')}</span>
                        )}
                      </div>
                    </td>
                  </tr>
              ...
              <tfoot>
                <tr className="border-t border-gray-600 bg-gray-800/30">
                  ...
                  <td className="p-3 text-center">
                    <span className={`text-xs font-medium ${leadTimeColor(longestLeadTime)}`}>{longestLeadTime}wk max</span>
                  </td>
                  <td />
                </tr>
              </tfoot>
      ...
      <div className="flex gap-4 text-xs">
        <span className="flex items-center gap-1"><span className="w-3 h-3 rounded bg-green-900/50 border border-green-600" /> &lt;2 weeks</span>
        <span className="flex items-center gap-1"><span className="w-3 h-3 rounded bg-yellow-900/50 border border-yellow-600" /> 2-8 weeks</span>
        <span className="flex items-center gap-1"><span className="w-3 h-3 rounded bg-red-900/50 border border-red-600" /> &gt;8 weeks</span>
      </div>
